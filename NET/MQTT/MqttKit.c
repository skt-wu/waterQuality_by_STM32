...
int32 MQTT_DumpLength(size_t len, uint8 *buf)
{
	...
	for(i = 1; i <= 4; ++i)				//例：len=200
	{
		*buf = len % 128;           //200%128=72   16进制为48		*buf=72
		len >>= 7;									//200 2进制数为 1100 1000					既做减法
		if(len > 0)									
		{
			*buf |= 128;							//做加法，最后用 48 01 发送出去
			++buf;										//自增
		}
		else
		{
...
int32 MQTT_ReadLength(const uint8 *stream, int32 size, uint32 *len)
{
	...
	int32 i;
	const uint8 *in = stream;										//定义中间变量
	uint32 multiplier = 1;

	*len = 0;
	for(i = 0; i < size; ++i)
	{
		*len += (in[i] & 0x7f) * multiplier;				//7f - 0111 1111 

		if(!(in[i] & 0x80))     //  判断 in [i] 中 某一个 第 8 位 为 0 (代表没有接下来的数据了)
		{
			return i + 1;
		}
						...
						MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	uint8 flags = 0;																			//用于接收byte8，判断有效载荷中数据是否存在，例如密码等
						...
						MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	//Publish消息---------------------------------------------------------------------------
	flags |= MQTT_PKT_PUBLISH << 4;             //MQTT控制报文类型
